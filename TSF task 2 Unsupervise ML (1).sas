...
FILENAME IRIS "/home/u57842000/sasuser.v94/Iris.csv";
/** Import the CSV file.  **/
...
RUN;
/** Print the results. **/
PROC PRINT DATA=WORK.IRIS (obs=15); 
RUN;
...
ods graphics on;
proc cluster data = work.iris method = centroid ccc print=15 outtree=Work.IRIS;
var sepallengthcm--petalwidthcm;
run;
ods graphics off;

/* Retaining 3 clusters */
proc tree noprint ncl=3 out=Work.IRIS;
copy sepallengthcm--petalwidthcm;
run;

/* To create a Scatterplot */
proc candisc out = IRIS;
class cluster;
...
scatter y = can2 x = can1 / group = cluster transparency=0.1;
run;
